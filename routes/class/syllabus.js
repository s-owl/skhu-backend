...
function processIntoUrl(rawTag, isOpened){
	...
	if(isOpened == "공개"){
		const rawstr = rawTag.split("&quot;"); // &quot 를 기준으로 쪼개기
		console.log(rawstr[1]);
		const data = rawstr[1].split("|"); // rawstr[1] 을 | 를 기준으로 쪼개서 배열로 저장
		console.log(data);
		// 쪼개기로 만들 배열의 요소와 문자열을 조합하여 URL 만들기
		const url = `${utils.forestBaseUrl}/Gate/SAM/Lesson/WEB/SSEW02O.aspx?Y=${data[10]}&HG=${data[11]}&GC=${data[12]}
